 ...
 */
function box(doc: Document, material: Material | null, centre: number[], half: number[], triangles = 12) {
  ...
  const primitive = doc
    .createPrimitive()
    .setAttribute("POSITION", doc.createAccessor().setType("VEC3").setArray(new Float32Array(corners)))
...
export function validModel(car: CarDefinition, spec: CarModelInterface) {
  ...
  const paint = doc.createMaterial("paint").setNormalTexture(texture("normal")).setOcclusionTexture(texture("ao"));
  const accent = doc
  ...
  const lods = (parent: Node, name: string, centre: number[], half: number[]) => {
    for (let i = 0; i < spec.lodCount; i += 1) {
      ...
      parent.addChild(doc.createNode(`${name}_LOD${String(i)}`).setMesh(box(doc, paint, centre, half, triangles)));
    }
  ...
  scene.addChild(lods(doc.createNode(spec.body), spec.body, [0, groundY + 0.5, 0.2], [0.8, 0.45, 2.55]));
  ...
  spec.wheels.forEach((name, i) => {
    ...
    const pivot = doc.createNode(name).setTranslation([x, w.connectionY - w.suspensionRestLength, z]);
    scene.addChild(lods(pivot, name, [0, 0, 0], [0.14, w.radius, w.radius]));
  });
  spec.flaps.forEach((name, i) => {
    const z = i === 0 ? 2.6 : -2.2;
    scene.addChild(lods(doc.createNode(name).setTranslation([0, 0, z]), name, [0, 0, 0], [0.7, 0.02, 0.1]));
  });
  ...
  scene.addChild(doc.createNode(spec.collision).setMesh(box(doc, null, [0, 0, 0], [h.x, h.y, h.z])));
