...
test("explains a Git LFS pointer instead of failing to parse it", () => {
  ...
  expect(() => assertGlb(bytes(pointer), "/assets/cars/fr26.glb")).toThrow(
    "/assets/cars/fr26.glb is a Git LFS pointer, not the model; run git lfs pull",
  );
});

test("rejects anything else, including an empty file", () => {
  expect(() => assertGlb(bytes("<!doctype html>"), "car.glb")).toThrow("car.glb is not a binary glTF (.glb) file");
  expect(() => assertGlb(bytes(""), "car.glb")).toThrow("car.glb is not a binary glTF (.glb) file");
});
